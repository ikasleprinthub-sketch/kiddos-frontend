...
export default function PopularBatters() {
  ...
  useEffect(() => {
    fetch("/api/products?popularBatter=true&limit=20")
      .then((r) => r.json())
      .then((data) => {
      ...
      <section className="bg-gray-50 py-10 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex-1 min-w-[175px] max-w-[175px] bg-white rounded-2xl h-64 animate-pulse shrink-0" />
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <section className="bg-gray-50 py-10 px-4 border-t border-gray-100">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8 pb-4 border-b border-zinc-100">
          <div>
            <h2 className="text-[#2a7a2a] font-black text-2xl tracking-tight">Popular Batters</h2>
            <p className="text-xs text-zinc-500 mt-1">Freshly stone-ground, naturally fermented batters</p>
          </div>
        </div>
